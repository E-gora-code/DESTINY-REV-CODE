...
class LocalizationTest extends OpMode {
    @Override
    public void init() {
        Tuning.follower.setStartingPose(new Pose(10.6,9.5));
    }
    ...
    public void loop() {
        ...
        Tuning.telemetryM.debug("y", Tuning.follower.getPose().getHeading());
        Tuning.telemetryM.update(telemetry);
...
class LateralTuner extends OpMode {
    ...
    public void init_loop() {
        ...
        Tuning.telemetryM.update(telemetry);
        Tuning.drawOnlyCurrent();
        dash.addData("x", Tuning.follower.getPose().getX());
        dash.update();
    }
...
class Line extends OpMode {
    ...
    @Override
    public void init() {
        Tuning.follower.setStartingPose(new Pose(72.057, 72.826));
    }
...
class Triangle extends OpMode {
    ...
    private final Pose interPose = new Pose(24 + 72, -24 + 72, Math.toRadians(0));
    private final Pose endPose = new Pose(24 + 72, 24 + 72, Math.toRadians(0));
